...
class RadDecay(object):
    ...
    # This is used to hold the values we need about this element  
    ...
    # This starts an loop to calculate the half life of the
    # element and then display it in an array form, as well as calculate the
    # simulated decay time
    def calculateDecay(self): 
        num_decayed = 0 
        out_array = np.zeros((self.arraySize,self.arraySize), dtype=int) #creating an numpy array of ints, to remove the . after each number
        radTime = 0.0

        while num_decayed < ((self.arraySize * self.arraySize) / 2): 
